 ...
 * - Search by class name/teacher/code
 * - Table listing classes with progress and actions
	...
	return (
		<div style={styles.page}>
			...
			<section style={styles.kpis}>
				...
				/>
				<StatCard icon={<IconUserGroup />} label="Tổng học viên" value={stats.totalStudents} />
						...
						<tbody>
							{filtered.map((c) => (
								<tr key={c.id} style={styles.tr}>
									...
									<td style={styles.td}>{c.startDate} - {c.endDate}</td>
									<td style={styles.td}>
...
function ViewClassModal({ cls, onClose }) {
	...
	function parseDMY(str) {
		...
		if (Number.isNaN(d) || Number.isNaN(m) || Number.isNaN(y)) return new Date(str);
	...
	const enrollmentRate = cls.students ? Math.round((cls.active / cls.students) * 100) : 0;
		...
		<div style={modalStyles.backdrop} role="dialog" aria-modal="true">
			<div style={modalStyles.container}>
				<div style={modalStyles.header}>
					...
					<button type="button" onClick={onClose} style={styles.iconButton} aria-label="Đóng">×</button>
				</div>
				<div style={modalStyles.body}>
					<div style={{ display: 'grid', gap: 12 }}>
						...
						<div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Mã lớp</div>
								<div style={{ fontWeight: 700 }}>{cls.code}</div>
							</div>
							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Giảng viên</div>
								<div style={{ fontWeight: 700 }}>{cls.teacher}</div>
							</div>

							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Sĩ số</div>
								<div style={{ fontWeight: 700 }}>{cls.students}</div>
							</div>
							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Hoạt động</div>
								<div style={{ fontWeight: 700 }}>{cls.active}</div>
							</div>

							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Tỉ lệ hoạt động</div>
								<div style={{ fontWeight: 700 }}>{enrollmentRate}%</div>
							</div>
							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Lịch học</div>
								<div style={{ fontWeight: 700 }}>{cls.schedule || '—'}</div>
							</div>

							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Thời gian</div>
								<div style={{ fontWeight: 700 }}>{cls.startDate} — {cls.endDate}</div>
								{durationDays !== null && <div style={{ marginTop: 6, color: '#6b7280', fontSize: 13 }}><strong>Thời lượng:</strong> {durationDays} ngày</div>}
							</div>

							<div style={{ padding: 12, background: '#fff', border: '1px solid #eef2f6', borderRadius: 10 }}>
								<div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>Trạng thái</div>
								<div><StatusBadge status={cls.status} /></div>
							</div>
						</div>
						...
						<div>
							<div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
								...
								<div style={{ minWidth: 56, textAlign: 'right', fontWeight: 700 }}>{cls.progress}%</div>
							</div>
				...
				<div style={modalStyles.footer}>
					<button type="button" onClick={onClose} style={modalStyles.ghostBtn}>Đóng</button>
				</div>
			</div>
		</div>
	);
...
function IconClass() {
	return (
		<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
			...
			<path d="M3 4h18M6 8h6M6 12h12M3 18h18" stroke="currentColor" strokeWidth="1.6" fill="none" />
		</svg>
	);
}
function IconCheckCircle() {
	return (
		<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
			...
			<path d="M8 12l3 3 5-6" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinecap="round" />
		</svg>
	);
}
...
function IconTrend() {
	return (
		<svg width="20" height="20" viewBox="0 0 24 24" fill="none">
			<path d="M4 20h16M5 14l4-4 4 3 6-7" stroke="currentColor" strokeWidth="1.6" fill="none" />
		</svg>
	);
}
function IconSearch() {
...
function IconCheckSmall() {
	return (
		<svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ marginRight: 6, color: "#22c55e" }}>
			<circle cx="12" cy="12" r="10" fill="#10b981" opacity="0.12" />
			<path d="M8 12l3 3 5-6" stroke="#16a34a" strokeWidth="1.8" fill="none" strokeLinecap="round" />
		</svg>
	);
}
function IconEye() {
