        ...
        const qs = data.questions.map((q, idx) => {
          ...
          const displayOpts = opts.map((opt) => {
            if (opt && typeof opt === "object") {
              const t = opt.text ?? opt.optionText ?? opt.value ?? opt.label;
              return t !== undefined ? String(t) : String(opt);
            }
            return String(opt);
          });
          let correctIndex = -1;
          ...
          if (Array.isArray(opts)) {
            const idxByFlag = opts.findIndex((opt) => opt && typeof opt === "object" && !!opt.isCorrect);
            if (idxByFlag >= 0) correctIndex = idxByFlag;
          }
          if (correctIndex < 0 && typeof ca === "string" && /^[A-D]$/i.test(ca)) {
            correctIndex = ca.toUpperCase().charCodeAt(0) - 65;
          }
          if (correctIndex < 0 && typeof ca !== "undefined") {
            const idxByValue = displayOpts.findIndex((t) => String(t).trim() === String(ca).trim());
            if (idxByValue >= 0) correctIndex = idxByValue;
          }
          return {
            id: q.id || idx + 1,
  ...
  const handleSubmit = useCallback(async (auto = false) => {
    ...
    try {
      ...
      const answersArr = usedQuestions.map((q) => {
        ...
        if (idx === undefined) return { questionId: q.id, answer: "" };
        // FIX: Send the actual answer TEXT, not the letter "A", "B"...
        // The backend requires the content to match (e.g. "Tomcat", not "A")
        const textAns = q.answers[idx] || "";
        return { questionId: q.id, answer: String(textAns) };
      });
